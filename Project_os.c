...
int nResources,//this represents the number of resources in the syhstem that the user can enter
    nProcesses;//this represnts the number of processes in the sytsem the user wants 
int *resources;//these are the array resources,allocated,max required and need
int **allocated;
int **maxRequired;
int **need;
int *safeSeq;
...
pthread_cond_t condition;//this lockResources and condition is necesary for allowing threada to work in a sequence

// get safe sequence if there is one else return false
bool getSafeSeq();
// process function
void* processCode(void* );//declared a funtion called processcode
int main(int argc, char** argv) {
	srand(time(NULL));//here we use the library for seed random to run different programs at different times
        ...
        allocated = (int **)malloc(nProcesses * sizeof(*allocated)); //allocated creates a 2d matrix to store the number of resources allocated
        for(int i=0; i<nProcesses; i++)
                allocated[i] = (int *)malloc(nResources * sizeof(**allocated));

        maxRequired = (int **)malloc(nProcesses * sizeof(*maxRequired));//this is a 2d matrix which takes the number of maximum resources 
        for(int i=0; i<nProcesses; i++)
